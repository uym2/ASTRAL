...
You simply need to download the [zip file](https://github.com/smirarab/ASTRAL/raw/master/Astral.5.12.7beta.zip)
...
```
java -Djava.library.path=. -jar astral.5.12.7beta.jar -i test_data/song_primates.424.gene.tre
```

This should quickly finish. There are also other sample input files under `test_data/` that can be used.

ASTRAL can be run from any directory. You just need to run `java -jar /path/to/astral/astral.5.12.7beta.jar`.
Also, you can move `astral.5.12.7beta.jar` to any location you like and run it from there, but note that you need
...
```
  java -Djava.library.path=. -jar astral.5.12.7beta.jar
```
...
```
java -Djava.library.path=. -jar astral.5.12.7beta.jar -i in.tree
```

The results will be outputted to the standard output. To save the results in a file use the `-o` option (**Strongly recommended**):

```
java -Djava.library.path=. -jar astral.5.12.7beta.jar -i in.tree -o out.tre
```
To save the logs (**also recommended**), run:

```
java -Djava.library.path=. -jar astral.5.12.7beta.jar -i in.tree -o out.tre 2>out.log
```
...
```
java -Djava.library.path=. -jar astral.5.12.7beta.jar -i best_ml -b bs_paths -r 100
```
...
```
java -Xmx3000M -Djava.library.path=. -jar astral.5.12.7beta.jar -i in.tree
```
